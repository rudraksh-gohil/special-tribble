...
class MongoDataUploadeR:
    ...
    def upload_data(self, collection_name):
        """Reads all JSON files starting with 'Merged_' from the schema directory and uploads data to MongoDB."""
        if not os.path.exists(self.schema_dir):
            logging.error(f"Schema directory '{self.schema_dir}' does not exist.")
            return

        for file_name in os.listdir(self.schema_dir):
            # Process only files that start with "Merged_" and have a .json extension
            if file_name.startswith('Merged_') and file_name.endswith('.json'):
...
def MongoDataUploader(MONGO_URI, DATABASE_NAME, FORMATTED_SCHEMA_DIR):
